...
function fortsetzen(){ stopFx(); $('start').classList.remove('show'); startOpen=false; paused=false; requestLock(); updateHUD(); }
/* Pausenmenue -> Startbildschirm: speichern und die gewohnte Auswahl
   zeigen (Weiterspielen / Neues Spiel) */
function zumStartbildschirm(){
  if(!S) return;
  save();
  pauseOpen=false; $('pause').classList.remove('show');
  if(locked) document.exitPointerLock();
  for(const k in keys) keys[k]=false; mouseDown=false;
  paused=true; startOpen=true; zeigeNamen(false); buildStart(); $('start').classList.add('show');
}
...
function buildStart(){
  ...
  /* Aus dem Spiel zurueck (Pausenmenue): Weiterspielen setzt das
     laufende Spiel fort, ohne neu zu laden */
  if(S) mk('Weiterspielen','',()=>fortsetzen());
  else if(loadSave()) mk('Weiterspielen','',()=>begin(false));
  mk('Neues Spiel',loadSave()?'ghost':'',()=>zeigeNamen(true));
  /* Ein neues Spiel mitten im laufenden braucht eine frische Welt:
     die Angaben merken, neu laden, dann startet es von selbst */
  $('nameGo').onclick=()=>{ const n=$('shopInput').value, sl=$('sloganInput').value, tu=$('tutInput').checked;
...
fontsReady().then(()=>{
  ...
  /* Neues Spiel aus dem laufenden heraus: nach dem Neuladen gleich los */
  { let ns=null; try{ ns=JSON.parse(sessionStorage.getItem('bb_neustart')||'null'); sessionStorage.removeItem('bb_neustart'); }catch(e){}
    if(ns) setTimeout(()=>begin(true,ns.n,ns.sl,ns.tu),0); }
